...
@Test("TerminalNotificationPreferenceStore defaults to true when unset and round-trips when set")
func terminalNotificationPreferenceStoreDefaultsTrueAndRoundTrips() {
    let suiteName = "TerminalAttentionTests.\(UUID().uuidString)"
    defer { UserDefaults(suiteName: suiteName)?.removePersistentDomain(forName: suiteName) }
    let store = TerminalNotificationPreferenceStore(suiteName: suiteName)
...
@MainActor
@Test("notifyIfNeeded posts exactly one matching notification for a belling, authorized session")
func notifyIfNeededPostsForBellingAuthorizedSession() async throws {
    ...
    let spy = SpyTerminalAttentionNotifier(authorized: true)
    session.attentionNotifier = spy

    session.notifyIfNeeded(for: controller)

    let posted = await waitUntilPosted(spy)
...
@MainActor
@Test("notifyIfNeeded posts nothing when the notification preference is off")
func notifyIfNeededPostsNothingWhenPreferenceOff() async throws {
    ...
    let store = TerminalNotificationPreferenceStore(suiteName: suiteName)
    store.setEnabled(false)
    session.terminalNotificationPreferenceStore = store
...
@MainActor
@Test("notifyIfNeeded posts nothing when the OS has not authorized notifications")
func notifyIfNeededPostsNothingWhenUnauthorized() async throws {
    ...
    let spy = SpyTerminalAttentionNotifier(authorized: false)
    session.attentionNotifier = spy
