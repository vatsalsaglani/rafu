...
/// `GeminiCLIAdapter` — OWNED BY PHASE C4
/// (`docs/plans/phases/conductor/C4-adapters-gemini-cursor.md`).
/// C4 REWRITES THIS FILE when it implements the adapter.
///
...
/// prevent.
///
/// The second test below is NOT a stub assertion and must survive C4's
/// rewrite.

@Test("GeminiCLIAdapter is registered and degrades honestly until C4 implements it")
func geminiCLIAdapterIsAnHonestStub() async {
    ...
    // A SUPERSET, not an exact key set: C4 may add whatever its own CLI
...
@Test("GeminiCLIAdapter agrees with itself about model discovery")
func geminiCLIAdapterDiscoveryFlagIsHonest() async {
