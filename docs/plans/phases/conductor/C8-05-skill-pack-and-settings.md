...
- **Status:** Ready. Branch: `conductor/c8-05-skill-pack` (from `main`
  AFTER C8-03 merged — verify `docs/references/ensemble-ipc-verbs.md`
  documents `run/abort/note/grant`; if it stops at the read-only verbs,
  STOP and report). Wave 3 — parallel with C8-04 and C8-07.
...
`references/verbs.md`: generated BY HAND from
`docs/references/ensemble-ipc-verbs.md` — same grammar lines, same JSON
examples, same state enum. Add a header: "Targets rafu ensemble verb
version 1. If `rafu ensemble status --json` reports a different
`verbVersion`, trust the CLI and re-read its --help."
...
to re-grant from the New Ensemble sheet).
...
- **Defaults card**: read-only display of the grant defaults
  (`ConductorEnsembleGrant` defaults from C8-03) with a caption that
  grants are set per-coordinator at launch — v1 surfaces, does not
  edit, defaults (recorded follow-up).
...
prerequisite: docs/references/ensemble-ipc-verbs.md documents the
mutating verbs run/abort/note/grant. If it stops at read-only verbs,
STOP and report — C8-03 has not merged.
...
HARD CONSTRAINTS: the skill must never contradict ensemble-ipc-verbs.md
(mirror, don't invent); the version-drift tripwire test
(targetsVerbVersion == ensembleVerbVersion) is mandatory; the installer
classifies every destination before writing anything and never silently
overwrites; never guess vendor skill directories beyond ~/.claude/skills
— the folder-picker is the honest fallback and the reference note says
so; Package.swift gets exactly one .copy line and nothing else; user-
visible strings say Ensemble; no credentials, no network, no I/O in
model construction. DO NOT touch engine files, ConductorRunDetailCanvas,
RafuAppCommands, CommandPaletteView, WorkspaceWindowView,
ConductorRunsPanelView, ConductorCore.swift, or script/ — C8-04/C8-07
own those this wave. HEADLESS ONLY.
