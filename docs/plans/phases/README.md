...
| [Workbench presentation upgrade — tucked depth, attached tabs, terminal tiles](workbench-presentation-upgrade.md) ([parallel execution](workbench-presentation-upgrade/README.md)) | Proposed (documentation and conflict-minimized worktree plans only, 2026-07-29); ADR 0022 acceptance required before implementation | Implemented UI refresh; current pre-initial-push workbench; ADR 0022 | One shared-foundation wave, six file-disjoint UI worktrees, and final integrated QA for the theme-preserving deck, tabs, terminal identity, utilities, Settings, and Ensemble |
...
| [Terminals as editor tabs](editor-terminal-tabs.md) | Historical predecessor; terminal-layout decisions resolved by ADR 0023 | ADR 0004/0014; ADR 0023 | Legacy tab placement remains compatible; Terminal Groups now own compound layout, inert restoration, capacity, and shortcut behavior |
| [Terminal manager — sessions panel, hide/close, shells, attention](terminal-manager.md) | Implemented (2026-07-21); session presentation history superseded for groups by ADR 0023 | ADR 0004/0014/0023; utility-rail pattern | The Terminal Manager retains session discovery, shell choice, identity, and attention; Terminal Groups now own group layout, park/reveal, and close lifecycle |
| [Terminal Groups — compound tabs and saved layouts](terminal-groups.md) ([worktree execution](terminal-groups/README.md)) | Active (2026-08-16); TG-00 complete on lane, TG-10 next after authorized merge | ADR 0023 | One serial decision plan, one serial source-contract plan, two file-disjoint parallel waves around one serial workspace cutover, and final integration QA for one named group tab, right/down panes, directional focus, bounded sessions, and inert saved layouts |
| [T-F — Notch Companion](terminal-notch-hud.md) | Implemented (2026-07-22): v1 attention HUD (seamless housing merge, OSC 9/777 + quiescence detection, snippet + reply) plus v2 companion NC-A…NC-E all shipped, advisor-reviewed, real on-notch-hardware GUI verified; VoiceOver-with-real-VoiceOver check owed | v1 HUD stack; ADR 0016 (amended); WorkspaceWindowRegistry | Always-on resting strip (editors count + attention dot), hover-peek panel (editors list w/ git one-liner, cross-window attention feed, reply), Codex %/Claude token usage tiles, "Show notch companion" toggle |
| [U — Agent usage providers](agent-usage-providers.md) | Planned (2026-07-22) | Notch Companion (shipped); new trust-transition ADR before U-B | Provider registry (descriptor + ordered strategies, CodexBar-MIT-inspired); exact Claude % via its own OAuth token; Cursor/OpenCode/Cline tiles; network providers strictly opt-in |
...
| [UX — Editor-hosted surfaces, theme fidelity, agent identity](ux/UX-execution-plan.md) | Planned (2026-07-26) | LT-01 (dogfooding in Lightning surfaced these) | No modal for New Ensemble / New Ensemble Run / Settings; no system-accent control where a theme token exists; correct vendor marks everywhere including where NSMenu prevented them; inline agent launcher; one `EditorCanvasRoute` instead of a 38-branch chain |
| [LT-01 — Rafu Lightning dev build](lightning-dev-build.md) | Merged 2026-07-26 (`06311ad`); guard found a 6th state site (ThemeFileService) beyond the plan's five | Wave-2 Ensemble merges on `main` | Local builds produce a silver-seam **Rafu Lightning** with its own process name, bundle id, Application Support root, Keychain service, IPC socket, and CLI symlink, so an agent's build/launch/kill cannot touch the Rafu the user is working in; releases stay non-Lightning via CI |
...
| [Ensemble — multi-provider agent orchestration](conductor/README.md) | **All phases C0–C7 merged to `main`** (2026-07-24/25); 1515 tests green, 0 warnings, GUI verified; 5 coordinator integration handoffs open (concurrency GUI ownership, usage persistence, recovery verbs, gate notification actions, process attribution — see the conductor README); renamed Conductor→Ensemble 2026-07-25 (ADR 0018 Naming); beyond-C7 orchestration gaps analyzed in `conductor/orchestration-gap-analysis.md` | ADR 0018; terminal manager + worktrees + diff canvas + usage registry (all shipped) | C0 shim serial, then C1 engine ∥ C2–C4 adapters (Claude Code, Codex, OpenCode, Cline, Kimi, Gemini, Cursor — all with model selection), then C5 pipelines, then C6 workflow library ∥ C7 polish; delegated auth, file handoffs, gated merge-back |
