...
| [Diff syntax highlighting + new-side hover](diff-syntax-highlighting-and-hover.md) | Planned (2026-07-20); to run in a dedicated worktree | Git experience baseline; ADR 0005 (opt-in LSP); tree-sitter grammar boundary | Diff canvas tokens colored both columns; hover (declarations/types) on the NEW side of working-tree diffs only — old-side hover is an explicit non-goal |
