...
| [LT-01 — Rafu Lightning dev build](lightning-dev-build.md) | Planned (2026-07-26); runs parallel with C8-04/C8-07 | Wave-2 Ensemble merges on `main` | Local builds produce a silver-seam **Rafu Lightning** with its own process name, bundle id, Application Support root, Keychain service, IPC socket, and CLI symlink, so an agent's build/launch/kill cannot touch the Rafu the user is working in; releases stay non-Lightning via CI |
