...
| [`conductor-file-contracts.md`](conductor-file-contracts.md) | Reading or writing `.rafu/`: layout and non-auto-invoked idempotent `seed()`, agent frontmatter tolerance with least-privilege `autonomy` defaulting, the workflow `[gate]`/`<-` grammar (and why not `!gate`), the hand-written strict-decoding `RunStepStatus` envelope for committable run evidence, and why `runDirectory` and `handoffDirectory` are both passed explicitly |
