...
| [`usage-provider-contract.md`](usage-provider-contract.md) | Writing or reviewing a `UsageFetchStrategy`/`UsageProviderDescriptor`: the `nonisolated protocol` requirement under default-`MainActor` isolation, `UsageHTTPError` redaction, `UsageSQLite` read-only/bound-params rule, the sync-credential-closure-over-async-actor bridge, and the `makeStrategies` context-independent-count rule |
