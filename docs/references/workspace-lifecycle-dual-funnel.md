...
Any work that must run when a window gains a workspace root (examples: reloading persisted Ensemble runs, priming a cache, refreshing external state) needs to be called from **both** functions.
...
- Workspaces that restore from a previous session lose workspace-specific state (e.g., Ensemble runs list is empty on window restore, even though `.rafu/runs/` exists on disk).
...
Verification (manual or automated): open a workspace with Ensemble runs, close the window, relaunch the app. The runs list populates on window restore, not just after explicit open.
